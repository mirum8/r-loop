...
in `rail.txt`, and the ASCII box fallback under **Shapes**. **A step's question text is never shown in
the TUI** (spec ADR-73), only the watchdog's own question to you: step agents ask the watchdog, and the watchdog asks the maintainer in its own
...
- **`secondary`** — amber. **Something is waiting for you**: a warning, or a watchdog asking you
  a question in its own pane (`watchdog waiting for you` in the header, the question in the feed). Questions and
