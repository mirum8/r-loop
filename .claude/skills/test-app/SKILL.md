...
**Never a live run inside r-loop's own run.** When `R_LOOP_RUN` is set (`[ -n "$R_LOOP_RUN" ]`) or the working directory is under `.r-loop/wt/` (`case "$PWD" in */.r-loop/wt/*)`), this skill was started by an r-loop step or reviewer session. That run is already live on this machine. A nested run would clash with its herdr agent names, and it would bring a second watchdog that asks the maintainer. Run the no-agent tier and the golden harness only. Report every live check as **NOT RUN (inside an r-loop run)**, and name the frames a manual `/test-app` should capture. Outside a run, a live run started by a subagent always passes `--unattended`, because no person watches a subagent's nested watchdog.
